...
const getMintStatus = async (clubId, dealId, userAddress) => {
  try {
    // Ensure TronLink is installed and user is logged in
    if (typeof window.tronWeb === "undefined") {
      toast.error("TronLink not found. Please install TronLink.");
    ...
    // Call the smart contract function to get minting status (assumes the function returns mint status)
    const mintStatus = await contract.getMintStatus(clubId, dealId, userAddress).call();

    // - currentMints: number of mints the user has already done
    // - maxMints: maximum number of mints allowed for this user
    const mintCount = mintStatus.mintCount.toNumber();
    const maxMints = mintStatus.maxMints.toNumber();

    console.log(`Mint Status for clubId ${clubId}, dealId ${dealId}:`, { mintCount, maxMints });

    return [mintCount, maxMints]; // Return mint counts for use in UI
  } catch (error) {
