...
const approveUSDSpend = async (
  ...
  try {
    ...
    const usdContract = await tronWeb.contract(
      usdContractABI,
      usdContractAddress
    );
    const currentAllowance = await usdContract
    ...
    console.log("Approving more USDT for spending...");
    const result = await usdContract.approve(spenderAddress, amount).send({
      feeLimit: 1000000000,
      callValue: 0,
    });
    const txID = result;
    console.log("USDT spend approved. Transaction ID:", txID);
    ...
    console.error("Error in approveUSDTSpend function:", error);
    throw error;
  }
};
