...
/**
 * Loads all deals for all clubs.
 *
 * @returns {Promise<Array<Object>>} - An array of deal objects.
 */
const loadAllDeals = async () => {
  ...
  try {
    ...
    for (let deal of deals_) {
      let onChainIdPair = await getDealIdFromEvent(deal.txID);
      let dealDetails = await getDealDetails(onChainIdPair.clubId, onChainIdPair.dealId);
      allDeals.push({
        _id: deal._id,
        description: deal.description,
...
export { createDeal, getDealIdFromEvent, getDealDetails, loadDealsForClub, loadAllDeals };
